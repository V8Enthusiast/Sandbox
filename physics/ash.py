    ...
    def calculate_physics(self):
        ...
        if self.y + 1 < self.simulation.ROWS and self.simulation.map[self.y + 1][self.x] in self.simulation.MOVING_SOLIDS and self.isFalling is False: # particle under is a moving one (sand)
            ...
            if right_below is False: # particle falls to the right
                self.simulation.map[self.y + 1][self.x + 1], self.simulation.map[self.y][self.x] = 9, self.simulation.map[self.y + 1][self.x + 1] # swap particles on the map
            ...
            #             self.y += 1
            #             break
            self.simulation.map[self.y + 1][self.x], self.simulation.map[self.y][self.x] = 9, self.simulation.map[self.y + 1][self.x]
            ...
            self.simulation.map[self.y + 1][self.x] = 9 # add the ash back 1 square lower
            self.simulation.particles[(self.x, self.y)] = None
